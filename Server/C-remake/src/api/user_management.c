...
Error create_user(sqlite3 **db, char *username, char *email, char *password) {
    ...
    Error status = insert_into_table(db, "users", "username, email, password", values);
    free(values);
    if(status.code != OK) {
...
Error delete_user(sqlite3 **db, char *userID) {
    int condition_length = strlen(userID) + 32; // +32 overhead for formatting
    char *condition = malloc(condition_length * sizeof(char));
    sprintf(condition, "userID = %s", userID);
    if(condition == NULL) {
