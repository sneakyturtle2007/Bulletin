...
int convert_table_to_string(Table_String *table, String *result){
  ...
  for(int i = 0; i < table->rows; i++){
    for(int k = 0; k < table->cols; k++){
      ...
      if(result->capacity < result->length + strlen(table->data[i][k].data) + 3){
        result->capacity *= 2;
        printf("string capacity %d", result->capacity);
      ...
      result->length += strlen(table->data[i][k].data);
  ...
  printf("Table converted to string successfully\n");
  printf("Table data: %s\n", result->data);
  return 0;
}
