    ...
    try{
      ...
      let onData = async (data) => {
        ...
        if(response.status == "Success" && testindex == 0){
          ...
          console.log(`addfriends|${userID}|admin,test,friend`); // Uses base database.db provided by the repository.
          client.write(`addfriends|${userID}|admin,test,friend`);// Same thing as the comment above ^^^
          testindex++;
