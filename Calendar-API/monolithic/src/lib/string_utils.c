...
Error strncpy_exclude(String *dest, char *src, const char *exclude){
  if(src == NULL || dest == NULL || exclude == NULL){
    fprintf(stderr, "ERROR: NULL parameter(s) string.\n");
    return (Error) {INVALID_ARGUMENT, "string_utils.c/strncpy_exclude/ERROR: Null parameter(s) string.\n"};
  }
  if(dest->data == NULL || dest->capacity < strlen(src) - strlen(exclude) + 1){
    //printf("strncpy_exclude: reallocating dest string.\n"); // DEBUG
    dest->capacity = strlen(src) - strlen(exclude) + 1; // +1 for null termination character
    ...
    if(dest->data == NULL){
      ...
      return (Error) {MEMORY_ALLOCATION_ERROR, 
                      "string_utils.c/strncpy_exclude/ERROR: Failed to allocate memory for dest string.\n"};
    }
  }
  if(strlen(src) == 0){
    //printf("strncpy_exclude: src is empty, nothing to exclude.\n"); // DEBUG
    return (Error) {OK, "Success"};
  }
  char *exclude_start= strstr(src, exclude);
  if(exclude_start == NULL){
    //printf("strncpy_exclude: exclude string not found in src, copying src to dest.\n"); // DEBUG
    return (Error) {OK, "Success"};
  }
  char *exclude_end = exclude_start + strlen(exclude);
  char *src_end = src + strlen(src);
  //printf("\nstrncpy_exclude\nexclude_start: %s\nsrc: %s\n", exclude_start, src); // DEBUG
  char *status = strncpy(dest->data, src, exclude_start - src);
  dest->length = (exclude_start - src);
  dest->data[dest->length] = '\0';\
  //printf("strncpy_exclude/dest->data: %s\n", dest->data); // DEBUG
  if(status == NULL){
  ...
  dest->length += (src_end - exclude_end);
  dest->data[dest->length] = '\0'; // null termination character
  //printf("strncpy_exclude: dest->data after exclude: %s\n", dest->data); // DEBUG
  if(status == NULL){
